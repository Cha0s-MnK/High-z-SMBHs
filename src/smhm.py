...
- masses are usually in Msun;
- times are tabulated internally in Myr unless a function says otherwise;
- the precomputed `zt`, `ct`, `ezt`, and `ozt` arrays are lookup tables for
  fast interpolation inside the formation/evolution loops.
"""

import numpy as np
from scipy import interpolate

# Use WMAP-9 cosmology, as adopted in the original Illustris-based model.
H0 = 7.195e-5 # h = 0.704 in Myr^-1
omega_matter = 0.2726
omega_lambda = 1. - omega_matter
ratio = omega_matter/omega_lambda
G = 0.00449972292 #pc^3 /(Msun * Myr^2)
# Behroozi+13 stellar-mass-to-halo-mass relation and supporting utilities.
def fSMHM(x, alpha, delta, gamma) : 
    """Nonlinear part of the Behroozi+13 SMHM relation."""
...
def SMHMparameters(z) : #standard Behroozi relation (Mvir)
    """Return redshift-dependent Behroozi+13 parameters for Mvir."""
...
def SMHMparameters2(z): #Kravtsov+ 2014 M200 (no scatter) 
    """Return the Kravtsov+14 M200 parameterization."""
...
def SMHMparameters3(z): #Kravtsov+ 2014 Mvir (no scatter)
    """Return the Kravtsov+14 Mvir parameterization."""
...
def SMHM(Mh, z, k = False, scatter = False, mdef = 'm200') : 
    """Convert halo mass to stellar mass using the selected SMHM relation."""
...
def lininterp(xarb,x, y, d_inv):
    """Fast linear interpolation on a uniformly spaced grid.

    Many older routines call this inside tight loops. It assumes `x` is evenly
    spaced and that callers already keep `xarb` inside the grid bounds.
    """
...
# Precompute cosmology tables once so later calls can interpolate instead of
# recomputing expensive transcendental functions inside model loops.
...
def cosmicTime(z, units = 'Myr') : #returns time(z) in Myr
    """Return cosmic time at redshift `z` using the precomputed table."""
...
def timeToRedshift(time, units = 'Myr') :    
    """Inverse of `cosmicTime` under the same background cosmology."""
...
def thub(z, units = 'yr'): #return 1/H(z) in years
    """Return the Hubble time at redshift `z`."""
...
def bulge_mass(sm, z): #M_bulge(M*, z), sm in log units
    """Approximate bulge mass from stellar mass and redshift."""
...
def find_nearest(array,value, return_idx = False):
    """Return nearest values or indices for scalar/array queries."""
...
def sample_power_law2(r, xmin, xmax, alpha): #more accurate version of above; alpha is the power-law index
    """Sample a bounded power law using the analytic inverse CDF."""
...
def kde_gauss(data, x, h, weights = None):
    """Simple Gaussian KDE used by a few legacy plotting utilities."""
